...
def index():
    try:
        ...
        if not user_uuid:
            ...
            # Log user creation
            logger.info(
                f"New user created: UUID={user_uuid}, Initial balance=10 {balance_type}"
            )
        ...
        if stored_user_agent is None or stored_user_agent.strip() != hashed_user_agent.strip():
            logger.info(
                f"User agent change detected for user {user_uuid}. Updating user agent."
            )
        ...
        logger.info(
            f"User {user_uuid} accessed. Current balance: {balance} {balance_type}."
        )
        ...
        logger.exception(f"Unexpected error in index route: {e}")
    ...
    try:
        ...
        if os.getenv("FLASK_ENV") == "development":
            ...
            if updated_balance is not None:
                logger.info(
                    f"{balance_to_add} {balance_type} added to user {user_uuid}. New balance: {updated_balance}."
                )
                flash(
                    f"{balance_to_add} {balance_type} has been added successfully. Current {balance_type}: {updated_balance}."
                )
            else:
                logger.error(
                    f"Failed to update {balance_type} for user {user_uuid}."
                )
        ...
        logger.exception(f"Unexpected error in buy_balance route: {e}")
        raise InternalServerError("An unexpected error occurred.")
...
def use_balance():
    try:
        ...
        if success:
            logger.info(
                f"{balance_type} used successfully for user {user_uuid}."
            )
            flash(f"{balance_type} used successfully!")
        else:
            logger.warning(
                f"Insufficient {balance_type} for user {user_uuid}."
            )
            flash(
                f"Insufficient {balance_type}. Please buy more or wait for free {balance_type}."
            )
        return redirect(url_for("index"))
    except Exception as e:
        logger.exception(f"Unexpected error in use_balance route: {e}")
        raise InternalServerError("An unexpected error occurred.")
...
def access_existing_balance():
    try:
        ...
        if database.check_uuid_exists(user_uuid):
            balance = database.get_balance(user_uuid)
            logger.info(
                f"Existing user {user_uuid} accessed. Current balance: {balance} {balance_type}."
            )
            ...
            logger.warning(
                f"Invalid UUID {user_uuid} attempted to access."
            )
        ...
        logger.exception(
            f"Unexpected error in access_existing_balance route: {e}"
        )
        raise InternalServerError("An unexpected error occurred.")
...
def clear_balance():
    if os.getenv("FLASK_ENV") == "development":
        try:
            ...
            logger.exception(
                f"Error clearing balance for user {user_uuid}: {e}"
            )
    ...
    if os.getenv("FLASK_ENV") == "development":
        try:
            ...
            if user_uuid:
                ...
                logger.info(
                    f"User record for UUID {user_uuid} deleted successfully."
                )
                response = make_response(redirect(url_for("index")))
            ...
            logger.exception(
                f"Error deleting user record for user {user_uuid}: {e}"
            )
...
def clear_all_balances():
    if os.getenv("FLASK_ENV") == "development":
        try:
            ...
            logger.exception(f"Error clearing all balances: {e}")
...
def delete_all_user_records():
    if os.getenv("FLASK_ENV") == "development":
        try:
            ...
            logger.exception(f"Error deleting all user records: {e}")
...
if __name__ == "__main__":
    # Only configure the logger in the main process
    if os.environ.get("WERKZEUG_RUN_MAIN") == "true" or not app.debug:
        # Configure Loguru to log to a file with rotation
        logger.add("logs/app_{time}.log", rotation="1 week")
    database.init_db()
